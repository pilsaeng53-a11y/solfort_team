...
const CALL_TABS = ["콜팀 현황", "콜팀 계정", "자동화", "조직도", "콜 자동화 현황"];
...
function CallAutomationLive() {
  ...
  const converted = leads.filter(l => l.status === "매출전환" && (l.converted_at || "").startsWith(todayStr));
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: "오늘 신규 리드", value: todayLeads.length, color: "text-blue-400" },
          { label: "오늘 콜 기록", value: todayLogs.length, color: "text-emerald-400" },
          { label: "관심 고객", value: interestLeads.length, color: "text-yellow-400" },
          { label: "오늘 매출전환", value: converted.length, color: "text-purple-400" },
        ].map(s => (
      ...
      <SFCard>
        <h3 className="text-xs font-semibold text-gray-400 mb-3">실시간 콜 기록 (10건)</h3>
        {todayLogs.length === 0 ? (
          <p className="text-xs text-gray-600 py-4 text-center">오늘 콜 기록 없음</p>
        ) : (
          <div className="space-y-0">
            {todayLogs.slice(0, 10).map(l => (
              <div key={l.id} className="flex items-center gap-3 py-2 border-b border-white/[0.04] last:border-0 text-xs">
                <span className="text-gray-600 text-[10px] w-12 shrink-0">{(l.called_at || "").substring(11, 16)}</span>
                <span className="text-gray-400 shrink-0">{l.called_by}</span>
      ...
      <div>
        <h3 className="text-sm font-semibold text-white mb-3">전체 리드 현황 ({leads.length}건)</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead><tr className="text-gray-500 border-b border-white/[0.06]">
              {["등록일", "고객명", "연락처", "담당자", "상태", "관심도", "다음콜"].map(h => (
            ...
            <tbody>
              {leads.slice(0, 50).map(l => (
                <tr key={l.id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                  ...
                  <td className="py-2.5 px-2">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] ${STATUS_COLORS_MAP[l.status] || "bg-white/5 text-gray-400"}`}>{l.status === "new" ? "신규" : l.status}</span>
                  </td>
